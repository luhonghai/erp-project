 ...
 * <p>Description: Detail controller used for sale invoice detail frame,
 * created from a set of out delivery notes,
 * related to the same sale document (order or contract).</p>
 ...
 * @version 1.0
 */
public class SaleInvoiceDocFromDelivNotesController extends CompanyFormController {

  /** sale invoice frame */
  private SaleInvoiceDocFromDelivNotesFrame frame = null;
  ...
  public SaleInvoiceDocFromDelivNotesController(SaleInvoiceDocsFrame parentFrame,SaleDocPK pk) {
    this.parentFrame = parentFrame;
    this.pk = pk;

    this.frame = new SaleInvoiceDocFromDelivNotesFrame(this);
  ...
  public Response insertRecord(ValueObject newPersistentObject) throws Exception {
    ...
    Response res = ClientUtils.getData(
        "createInvoiceFromOutDelivNotes",
        new SaleInvoiceFromDelivNotesVO((DetailSaleDocVO)newPersistentObject,rows)
    );
    if (!res.isError()) {
  ...
  public void afterInsertData() {
    ...
    frame.getDelivPanel().getGrid().getOtherGridParams().put(ApplicationConsts.SALE_DOC_VO,vo);
    frame.getDelivPanel().getGrid().setMode(Consts.READONLY);
    frame.getDelivPanel().getGrid().clearData(); // clear grid content...

    frame.setButtonsEnabled(true);
    frame.enabledConfirmButton();
  ...
  public void createPersistentObject(ValueObject persistentObject) throws Exception {
    ...
    vo.setDocTypeDOC01(ApplicationConsts.SALE_INVOICE_FROM_DN_DOC_TYPE);
