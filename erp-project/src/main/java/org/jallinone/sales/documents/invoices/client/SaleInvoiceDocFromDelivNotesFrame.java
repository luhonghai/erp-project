 ...
 * <p>Description: Detail Frame used for a sale invoice, created from a set of out delivery notes,
 * related to the same sale document (contract or order).</p>
 ...
 * @version 1.0
 */
public class SaleInvoiceDocFromDelivNotesFrame extends InternalFrame implements InvoiceDocument {

  /** detail form controller */
  private SaleInvoiceDocFromDelivNotesController controller = null;
  ...
  SaleInvoiceDocRowsGridPanel rowsPanel = new SaleInvoiceDocRowsGridPanel(this,headerFormPanel,false,false);
  ...
  private OutDeliveryNotesFromSaleDocPanel delivPanel = new OutDeliveryNotesFromSaleDocPanel(this);
  ...
  public SaleInvoiceDocFromDelivNotesFrame(SaleInvoiceDocFromDelivNotesController controller) {
    this.controller = controller;
    try {
      jbInit();
      setSize(750,630);
      setMinimumSize(new Dimension(750,630));
      setTitle(ClientSettings.getInstance().getResources().getResource("sale invoice from delivery notes"));
...
//      saleCustomerHeadPanel1.getCustomerDataLocator().getLookupValidationParameters().put(ApplicationConsts.SUBJECT_TYPE,ApplicationConsts.SUBJECT_PEOPLE_CUSTOMER);
  ...
  private void jbInit() throws Exception {
    ...
    printButton.addActionListener(new SaleInvoiceDocFromDelivNotesFrame_printButton_actionAdapter(this));

    saleIdHeadPanel1.getControlDocRifLookup().setRequired(true);

    addPanel.setLayout(borderLayout5);
    confirmButton.addActionListener(new SaleInvoiceDocFromDelivNotesFrame_confirmButton_actionAdapter(this));

    headerFormPanel.setFunctionId("DOC01_INV_FROM_DN");
    ...
    tabbedPane.setTitleAt(2,ClientSettings.getInstance().getResources().getResource("delivery notes"));
    tabbedPane.setTitleAt(3,ClientSettings.getInstance().getResources().getResource("items"));
    tabbedPane.setTitleAt(4,ClientSettings.getInstance().getResources().getResource("customer discounts"));
    tabbedPane.setTitleAt(5,ClientSettings.getInstance().getResources().getResource("charges"));
    tabbedPane.setTitleAt(6,ClientSettings.getInstance().getResources().getResource("sale activities"));
  ...
  public OutDeliveryNotesFromSaleDocPanel getDelivPanel() {
    return delivPanel;
  }
...
class SaleInvoiceDocFromDelivNotesFrame_confirmButton_actionAdapter implements java.awt.event.ActionListener {
  SaleInvoiceDocFromDelivNotesFrame adaptee;

  SaleInvoiceDocFromDelivNotesFrame_confirmButton_actionAdapter(SaleInvoiceDocFromDelivNotesFrame adaptee) {
...
class SaleInvoiceDocFromDelivNotesFrame_printButton_actionAdapter implements java.awt.event.ActionListener {
  SaleInvoiceDocFromDelivNotesFrame adaptee;

  SaleInvoiceDocFromDelivNotesFrame_printButton_actionAdapter(SaleInvoiceDocFromDelivNotesFrame adaptee) {
    this.adaptee = adaptee;
  }
  public void actionPerformed(ActionEvent e) {
