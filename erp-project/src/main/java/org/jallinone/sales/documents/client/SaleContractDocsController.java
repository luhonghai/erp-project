 ...
 * <p>Description: Grid controller used for sale contracts grid.</p>
 ...
 * @version 1.0
 */
public class SaleContractDocsController extends CompanyGridController {

  /** sale contracts frame */
  private SaleContractDocsFrame frame = null;


  public SaleContractDocsController() {
    this.frame = new SaleContractDocsFrame(this);
  ...
  public boolean beforeInsertGrid(GridControl grid) {
    ...
    if (canIns) {
      // create sale order detail frame in INSERT mode...
      new SaleContractDocController(frame,null);
    }
    return false;
  }
  ...
  public void doubleClick(int rowNumber,ValueObject persistentObject) {
    ...
    new SaleContractDocController(frame,pk);
  }
  ...
  public boolean beforeDeleteGrid(GridControl grid) {
    ...
    for(int i=0;i<frame.getGrid().getSelectedRows().length;i++) {
      ...
      if (vo.getDocStateDOC01().equals(ApplicationConsts.CONFIRMED))
        return false;
      else if (vo.getDocStateDOC01().equals(ApplicationConsts.CLOSED))
        return false;
    }
    return true;
  }
