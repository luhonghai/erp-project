 ...
 * <p>Description: Grid controller used for sale invoice rows grid.</p>
 ...
 * @version 1.0
 */
public class SaleInvoiceDocRowsController extends CompanyGridController {

  /** sale invoice rows panel */
  private SaleInvoiceDocRowsGridPanel panel = null;


  public SaleInvoiceDocRowsController(SaleInvoiceDocRowsGridPanel panel) {
    this.panel = panel;
  }
  ...
  public boolean beforeInsertGrid(GridControl grid) {
    if (!super.beforeInsertGrid(grid))
      return false;
    if (panel.getParentVO().getDocStateDOC01().equals(ApplicationConsts.CLOSED))
      return false;
  ...
  public void doubleClick(int rowNumber,ValueObject persistentObject) {
    ...
    ((SaleInvoiceDocRowController)panel.getDetailPanel().getFormController()).setPk(pk);
