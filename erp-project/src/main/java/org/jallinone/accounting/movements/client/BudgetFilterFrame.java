 ...
 * <p>Description: Filter frame used to view the budget filter frame.</p>
 ...
 * @version 1.0
 */
public class BudgetFilterFrame extends InternalFrame {
  ...
  public BudgetFilterFrame() {
    try {
      ...
      ArrayList companiesList = bca.getCompaniesList("ACC06_BUDGET");
      Domain domain = new Domain("DOMAIN_ACC06_BUDGET");
      for (int i = 0; i < companiesList.size(); i++) {
        if (applet.getAuthorizations().getCompanyBa().isInsertEnabled(
            "ACC06_BUDGET",companiesList.get(i).toString()
        ))
  ...
  private void jbInit() throws Exception {
    printButton.setToolTipText(ClientSettings.getInstance().getResources().getResource("print budget"));
    printButton.addActionListener(new BudgetFilterFrame_printButton_actionAdapter(this));
    controlCompaniesCombo.setAttributeName("companyCodeSys01WAR01");

    this.setTitle(ClientSettings.getInstance().getResources().getResource("print budget"));
  ...
  void printButton_actionPerformed(ActionEvent e) {
    ...
    params.put("OPENING_ACCOUNTING_MOTIVE_CODE",ApplicationConsts.MOTIVE_ACCOUNTS_OPENING);

    HashMap map = new HashMap();
    map.put(ApplicationConsts.COMPANY_CODE_SYS01,controlCompaniesCombo.getValue());
    map.put(ApplicationConsts.FUNCTION_CODE_SYS06,"ACC06_BUDGET");
    ...
    if (!res.isError()) {
      ...
      frame.setVisible(true);
        ...
        ClientUtils.getParentFrame(this),
        res.getErrorMessage(),
        ClientSettings.getInstance().getResources().getResource("print budget"),
...
class BudgetFilterFrame_printButton_actionAdapter implements java.awt.event.ActionListener {
  BudgetFilterFrame adaptee;

  BudgetFilterFrame_printButton_actionAdapter(BudgetFilterFrame adaptee) {
