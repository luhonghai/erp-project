...
public class InsertItemAction implements Action {
  ...
  public final Response executeCommand(Object inputPar,UserSessionParameters userSessionPars,HttpServletRequest request, HttpServletResponse response,HttpSession userSession,ServletContext context) {
	  ...
	  try {
		  ...
		  ArrayList customizedFields = cust.getCustomizedFields(vo.getProgressiveHie01HIE02()); // lob 
		  
		  ServerResourcesFactory factory = (ServerResourcesFactory)context.getAttribute(Controller.RESOURCES_FACTORY);
		  Resources res = factory.getResources(userSessionPars.getLanguageId());
		  String t1 = res.getResource("barcode already assigned to another item");
		  ...
		  InsertItem bean = (InsertItem)JAIOBeanFactory.getInstance().getBean(InsertItem.class);
		  Response answer = bean.insertItem(vo,t1,((JAIOUserSessionParameters)userSessionPars).getServerLanguageId(),userSessionPars.getUsername(),imagePath,companiesList,customizedFields);
