...
public class LoadItemTypesAction implements Action {
  ...
  public final Response executeCommand(Object inputPar,UserSessionParameters userSessionPars,HttpServletRequest request, HttpServletResponse response,HttpSession userSession,ServletContext context) {
	  ...
	  try {
		  ...
		  LoadItemTypes bean = (LoadItemTypes)JAIOBeanFactory.getInstance().getBean(LoadItemTypes.class);
		  ArrayList companiesList = ((JAIOUserSessionParameters)userSessionPars).getCompanyBa().getCompaniesList("ITM02");
		  Response answer = bean.loadItemTypes(gridParams,((JAIOUserSessionParameters)userSessionPars).getServerLanguageId(),userSessionPars.getUsername(),companiesList,customizedFields);
