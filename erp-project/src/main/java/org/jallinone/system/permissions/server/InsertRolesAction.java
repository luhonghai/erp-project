...
import org.jallinone.system.server.UserAuthorizations;
...
public class InsertRolesAction implements Action {
	...
	public final Response executeCommand(Object inputPar,
		...
		try {
			...
			Response answer = bean.insertRoles(list,((JAIOUserSessionParameters)userSessionPars).getServerLanguageId(),userSessionPars.getUsername(),defCompanyCodeSys01SYS03);

			
	        // update user roles collection...
			UserAuthorizations authBean = (UserAuthorizations)JAIOBeanFactory.getInstance().getBean(UserAuthorizations.class);
			HashMap userRoles = authBean.getUserRoles(((JAIOUserSessionParameters)userSessionPars).getServerLanguageId(),userSessionPars.getUsername());
