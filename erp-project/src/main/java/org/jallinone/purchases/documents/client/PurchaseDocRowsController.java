 ...
 * <p>Description: Grid controller used for purchase order rows grid.</p>
 ...
 * @version 1.0
 */
public class PurchaseDocRowsController extends CompanyGridController {

  /** purchase order rows panel */
  private PurchaseDocRowsGridPanel panel = null;


  public PurchaseDocRowsController(PurchaseDocRowsGridPanel panel) {
    this.panel = panel;
  }
  ...
  public boolean beforeInsertGrid(GridControl grid) {
    if (!super.beforeInsertGrid(grid))
      return false;
    if (panel.getParentVO().getDocStateDOC06().equals(ApplicationConsts.CONFIRMED))
      return false;
  ...
  public void doubleClick(int rowNumber,ValueObject persistentObject) {
    ...
    ((PurchaseDocRowController)panel.getDetailPanel().getFormController()).setPk(pk);
