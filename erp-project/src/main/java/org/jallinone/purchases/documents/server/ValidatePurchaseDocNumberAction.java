...
public class ValidatePurchaseDocNumberAction implements Action {
  ...
  public final Response executeCommand(Object inputPar,UserSessionParameters userSessionPars,HttpServletRequest request, HttpServletResponse response,HttpSession userSession,ServletContext context) {
    ...
    try {
      ...
      ValidatePurchaseDocNumber bean = (ValidatePurchaseDocNumber)JAIOBeanFactory.getInstance().getBean(ValidatePurchaseDocNumber.class);
      ArrayList companiesList = ((JAIOUserSessionParameters)userSessionPars).getCompanyBa().getCompaniesList("DOC06_ORDERS");
      Response answer = bean.validatePurchaseDocNumber(pars,((JAIOUserSessionParameters)userSessionPars).getServerLanguageId(),userSessionPars.getUsername(),companiesList);
