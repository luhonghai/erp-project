 ...
 * created from a set of in delivery notes,
 * related to the same purchase document (order or contract).</p>
 ...
 * @version 1.0
 */
public class PurchaseInvoiceDocFromDelivNotesController extends CompanyFormController {
  ...
  public void afterInsertData() {
    ...
    frame.getDelivPanel().getGrid().getOtherGridParams().put(ApplicationConsts.PURCHASE_DOC_VO,vo);
    frame.getDelivPanel().getGrid().setMode(Consts.READONLY);
    frame.getDelivPanel().getGrid().clearData(); // remove grid content...
