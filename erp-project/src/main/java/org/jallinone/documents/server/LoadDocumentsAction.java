...
public class LoadDocumentsAction implements Action {
	...
	public final Response executeCommand(Object inputPar,UserSessionParameters userSessionPars,HttpServletRequest request, HttpServletResponse response,HttpSession userSession,ServletContext context) {
		...
		try {
			...
			HashMap filters = (HashMap)pars.getOtherGridParams().get(ApplicationConsts.PROPERTIES_FILTER);
			pars.getOtherGridParams().remove(ApplicationConsts.PROPERTIES_FILTER);
			Response answer = bean.loadDocuments(pars,filters,((JAIOUserSessionParameters)userSessionPars).getServerLanguageId(),userSessionPars.getUsername(),companiesList);
