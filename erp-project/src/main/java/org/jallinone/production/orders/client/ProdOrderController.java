 ...
 * <p>Description: Detail controller used for production order detail frame.</p>
 ...
 * @version 1.0
 */
public class ProdOrderController extends CompanyFormController {

  /** production order frame */
  ...
  public Response loadData(Class valueObjectClass) {
    return ClientUtils.getData("loadProdOrder",pk);
  }
  ...
  public Response insertRecord(ValueObject newPersistentObject) throws Exception {
    Response res = ClientUtils.getData("insertProdOrder",newPersistentObject);
    if (!res.isError()) {
      DetailProdOrderVO vo = (DetailProdOrderVO)((VOResponse)res).getVo();
      pk = new ProdOrderPK(
          vo.getCompanyCodeSys01DOC22(),
          vo.getDocYearDOC22(),
          vo.getDocNumberDOC22()
      );
      frame.getProductsGrid().getOtherGridParams().put(ApplicationConsts.PROD_ORDER_PK,pk);
    }
    return res;
  }
...
//    frame.getProductsPanel().setParentVO(vo);
//    frame.getProductsPanel().getGrid().getOtherGridParams().put(ApplicationConsts.PURCHASE_DOC_PK,pk);
//    frame.getProductsPanel().getGrid().reloadData();
  ...
  public void createPersistentObject(ValueObject persistentObject) throws Exception {
    ...
    vo.setDocYearDOC22(new BigDecimal(cal.get(cal.YEAR)));
    vo.setDocDateDOC22(new java.sql.Date(System.currentTimeMillis()));
    vo.setDocStateDOC22(ApplicationConsts.OPENED);
  }
