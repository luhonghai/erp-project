 ...
 * @version 1.0
 */
public class OutDeliveryNoteController extends CompanyFormController {

  /** delivery note frame */
  private OutDeliveryNoteFrame frame = null;

  /** parent frame */
  private OutDeliveryNotesFrame parentFrame = null;

  /** delivery note pk */
  private DeliveryNotePK pk = null;


  public OutDeliveryNoteController(OutDeliveryNotesFrame parentFrame,DeliveryNotePK pk) {
    this.parentFrame = parentFrame;
    this.pk = pk;
    this.frame = new OutDeliveryNoteFrame(this);
    MDIFrame.add(frame);
  ...
  public Response loadData(Class valueObjectClass) {
    return ClientUtils.getData("loadOutDeliveryNote",pk);
  }
  ...
  public Response insertRecord(ValueObject newPersistentObject) throws Exception {
    Response res = ClientUtils.getData("insertOutDeliveryNote",newPersistentObject);
    if (!res.isError()) {
  ...
  public Response updateRecord(ValueObject oldPersistentObject,ValueObject persistentObject) throws Exception {
    Response res = ClientUtils.getData("updateOutDeliveryNote",new ValueObject[]{oldPersistentObject,persistentObject});
    if (!res.isError()) {
      if (parentFrame!=null) {
        parentFrame.getGrid().reloadCurrentBlockOfData();
        DetailDeliveryNoteVO vo = (DetailDeliveryNoteVO)frame.getHeaderFormPanel().getVOModel().getValueObject();
  ...
  public void createPersistentObject(ValueObject persistentObject) throws Exception {
    ...
    vo.setDocTypeDOC08(ApplicationConsts.OUT_DELIVERY_NOTE_DOC_TYPE);
  ...
  public OutDeliveryNotesFrame getParentFrame() {
    return parentFrame;
  }
